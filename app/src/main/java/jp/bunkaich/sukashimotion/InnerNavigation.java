...
final class InnerNavigation {
    ...
    private LinearLayout buttons(){
        LinearLayout row=new LinearLayout(context);row.setGravity(Gravity.CENTER);row.setPadding(dp(8),dp(4),dp(8),dp(4));row.setBackground(background(0xf2f6faf8,26));
    ...
    void showRecent(List<Bundle> apps){
        ...
        LinearLayout panel=new LinearLayout(context);panel.setOrientation(LinearLayout.VERTICAL);panel.setPadding(dp(14),dp(14),dp(14),dp(8));panel.setBackground(background(0xfff6faf8,26));
        LinearLayout heading=new LinearLayout(context);heading.setGravity(Gravity.CENTER_VERTICAL);
        TextView title=new TextView(context);title.setText(context.getString(R.string.nav_recents));title.setTextSize(19);title.setTextColor(0xff17201c);heading.addView(title,new LinearLayout.LayoutParams(0,dp(40),1));
        ...
        if(apps.isEmpty()){TextView empty=new TextView(context);empty.setText(context.getString(R.string.no_recent_apps));empty.setTextColor(0xff17201c);empty.setPadding(dp(12),dp(32),dp(12),dp(32));list.addView(empty);}
        ...
        for(int i=0;i<apps.size();i++){
            ...
            LinearLayout card=new LinearLayout(context);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(dp(8),dp(8),dp(8),dp(8));card.setBackground(background(0xffffffff,16));
            ImageView preview=new ImageView(context);preview.setImageBitmap(app.getParcelable("icon",Bitmap.class));preview.setScaleType(ImageView.ScaleType.CENTER_INSIDE);preview.setBackgroundColor(0xffe8efeb);card.addView(preview,new LinearLayout.LayoutParams(-1,dp(160)));previews.put(task,preview);
            TextView name=new TextView(context);name.setText(label);name.setTextSize(15);name.setTextColor(0xff17201c);name.setMaxLines(1);name.setEllipsize(android.text.TextUtils.TruncateAt.END);name.setPadding(dp(4),dp(10),dp(4),dp(4));card.addView(name);
    ...
    private static final class NavButton extends View {
        ...
        protected void onDraw(Canvas canvas){super.onDraw(canvas);float cx=getWidth()/2f,cy=getHeight()/2f,r=8*density;p.setColor(0xff17201c);p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.7f*density);p.setStrokeCap(Paint.Cap.ROUND);
